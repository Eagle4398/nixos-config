...
let
  ...
  readLinesFromFile = path:
    let
      ...
      filteredLines = lib.lists.filter
        (line:
        ...
        lines;
    in
    lib.lists.map (line: lib.strings.trim line) filteredLines;

  # This is for parsing "export SSH_AUTH_SOCK=..." etc
  regex = "^[[:space:]]*(export[[:space:]]+)?([a-zA-Z_][a-zA-Z0-9_]*)[[:space:]]*=[[:space:]]*(.*)$";
      ...
      (acc: line:
        ...
        if match == null then
          ...
          let
            ...
            value = lib.strings.replaceStrings [ "$HOME" ] [ "${config.home.homeDirectory}" ] rawValue;
          in
  ...
  pathsToAdd = lib.lists.map
    (path: lib.strings.replaceStrings
      [ "$HOME" "$BASHRC" ]
      [ "${config.home.homeDirectory}" "${toString ./dotfiles}" ]
      path
    )
    pathLines;
...
in
{
  ...
  home.sessionVariables = exportAttributes // { DOTFILES = "${toString ./dotfiles}"; };
